            ...
            <div className="flex gap-6 h-[400px]">
                ...
                <div className="flex-[2] bg-white rounded-3xl border border-slate-200 flex flex-col p-8 relative overflow-hidden group">
                    ...
                    <div className="flex flex-col flex-1 justify-center relative z-10 pl-2">
                        ...
                        <div className="h-24">
                            <AnimatePresence mode="wait">
                                <motion.h3 key={currentIndex} initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -10, opacity: 0 }} className="text-6xl font-black text-[#1d2d6a] tracking-tighter">{currentStation}</motion.h3>
                            </AnimatePresence>
                        </div>
                    </div>
                    <div className="flex items-center gap-4 mt-auto relative z-10">
                        ...
                        <button onClick={onSync} className="flex-1 h-16 bg-[#ee6f1f] hover:bg-[#d86116] text-white rounded-2xl font-black shadow-lg transition-all active:scale-95 flex items-center justify-center gap-3 drop-shadow-sm text-lg"><RefreshCcw size={22} /> SYNC DISPLAY STATUS</button>
                ...
                <div className="flex-1 bg-white rounded-3xl border border-slate-200 flex flex-col items-center justify-center p-8 relative overflow-hidden group">
                    ...
                    <div className="flex flex-col items-center justify-center relative z-10">
                        <div className="h-16 mb-2 flex items-center justify-center">
                            <AnimatePresence mode="wait">
                                <motion.h4 key={currentIndex} initial={{ y: 5, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -5, opacity: 0 }} className="text-4xl font-black text-[#1d2d6a] tracking-tighter truncate text-center" title={nextStation}>{nextStation}</motion.h4>
                            </AnimatePresence>
                        </div>
