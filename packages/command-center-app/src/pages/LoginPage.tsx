...
export default function LoginPage({ onLogin }: { onLogin: (user: AuthUser, token: string) => void }) {
    ...
    return (
        <div className="relative flex h-screen w-full items-center justify-center overflow-hidden bg-white font-sans">
            <div className="absolute inset-0 z-0 scale-105" style={{ backgroundImage: "url('https://ik.trn.asia/uploads/2022/11/1669271102786.jpeg')", backgroundSize: 'cover', backgroundPosition: 'center', filter: 'blur(3px) brightness(1.1)' }} />
            ...
            <div className="relative z-10 flex w-full items-center justify-center px-4">
                <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.6, ease: "easeOut" }}
                    ...
                    <form onSubmit={submit} className="flex w-full flex-col space-y-7">
                        ...
                        <div className="relative w-full">
                            <input type={showPass ? 'text' : 'password'} placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} className="w-full border-b-[1.5px] border-slate-200 bg-transparent py-2.5 pr-10 text-sm font-medium text-slate-800 placeholder-slate-400 focus:border-[#ee6f1f] focus:outline-none transition-colors" disabled={loading} autoComplete="current-password" />
                            <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-0 top-[10px] text-slate-400 hover:text-slate-600 transition-colors" tabIndex={-1}>
