    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl overflow-visible flex flex-col"
        >
          ...
          <div className="p-8 flex flex-col gap-8 bg-[#f8fafc]">
            <div className="flex flex-col gap-3">
              <label className="text-xs font-bold text-[#1d2d6a] pl-1 uppercase tracking-wider">
                Select Route / Train Name
              </label>
              <div className="relative flex-1" ref={serviceDropdownRef}>
                <div
                  className={`w-full bg-white border-2 rounded-xl px-5 py-4 shadow-sm transition-all cursor-pointer flex items-center ${serviceDropdownOpen ? "border-[#ee6f1f] ring-4 ring-orange-500/10" : "border-slate-200 hover:border-slate-300"}`}
                ...
                >
                  <input
                    ...
                    className="flex-1 bg-transparent border-none focus:ring-0 focus:outline-none text-base font-bold text-[#1d2d6a] placeholder-slate-400 p-0"
                  />
                  <ChevronDown
                    size={20}
                    strokeWidth={2.5}
                    ...
                    {filtered.length === 0 ? (
                      ...
                      filtered.map((t) => (
                        <div
                          ...
                          className={`px-5 py-3 text-base font-bold cursor-pointer transition-colors ${trainNameIndex === t.idx ? "bg-[#ee6f1f]/10 text-[#ee6f1f]" : "text-[#1d2d6a] hover:bg-slate-50"}`}
                        >
                          {t.name}
                        </div>
                      ))
