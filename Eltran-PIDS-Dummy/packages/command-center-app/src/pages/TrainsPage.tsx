                ...
                {showForm && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm space-y-8 overflow-hidden">
                        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                            <h3 className="text-[#1d2d6a] font-black text-lg flex items-center gap-2"><Plus size={20} className="text-[#ee6f1f]" /> Registrasi Kereta & Gerbong</h3>
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 px-3 py-1 rounded-lg">Konfigurasi Armada</span>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-6">
                                <div className="space-y-4">
                                    <h4 className="font-black text-[#1d2d6a] text-xs flex items-center gap-2 border-l-4 border-[#ee6f1f] pl-3 uppercase tracking-wider">Identitas utama</h4>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Nama Kereta</label>
                                            <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value.toUpperCase() })} placeholder="e.g. ARGO WILIS" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-[#1d2d6a] font-bold text-sm focus:outline-none focus:border-[#ee6f1f] transition-all" />
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Kode Kereta (KA)</label>
                                            <input value={form.ka_number} onChange={e => setForm({ ...form, ka_number: e.target.value.toUpperCase() })} placeholder="e.g. 1A" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-[#1d2d6a] font-bold text-sm focus:outline-none focus:border-[#ee6f1f] transition-all" />
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Stasiun Awal</label>
                                            <select value={form.stasiun_awal} onChange={e => setForm({ ...form, stasiun_awal: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-[#1d2d6a] font-bold text-sm focus:outline-none focus:border-[#ee6f1f] appearance-none transition-all">
                                                <option value="">Pilih Stasiun</option>
                                                {stations.map(s => <option key={s.id} value={s.id}>{s.name} ({s.id})</option>)}
                                            </select>
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Stasiun Akhir</label>
                                            <select value={form.stasiun_akhir} onChange={e => setForm({ ...form, stasiun_akhir: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-[#1d2d6a] font-bold text-sm focus:outline-none focus:border-[#ee6f1f] appearance-none transition-all">
                                                <option value="">Pilih Stasiun</option>
                                                {stations.map(s => <option key={s.id} value={s.id}>{s.name} ({s.id})</option>)}
                                            </select>
                                        </div>
                                ...
                                <div className="space-y-4">
                                    <h4 className="font-black text-[#1d2d6a] text-xs flex items-center gap-2 border-l-4 border-[#ee6f1f] pl-3 uppercase tracking-wider">Rute Stasiun (Stasiun Antara)</h4>
                                    <div className="relative">
                                        <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none text-slate-400"><MapPin size={16} /></div>
                                        <input value={stationSearch} onChange={e => setStationSearch(e.target.value)} placeholder="Cari stasiun antara..." className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-12 pr-4 py-3 text-[#1d2d6a] font-bold text-sm focus:outline-none focus:border-[#ee6f1f] transition-all" />
                                            ...
                                            <div className="absolute z-10 w-full mt-2 bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden divide-y divide-slate-100 max-h-[200px] overflow-y-auto">
                                                {filteredSuggestions.map((s, idx) => (
                                                    <button key={idx} onClick={() => addStationToRoute(s.name)} className="w-full px-4 py-3 text-left hover:bg-slate-50 transition-colors flex items-center justify-between group">
                                                        <div><div className="text-[#1d2d6a] font-bold text-sm">{s.name}</div><div className="text-slate-400 text-[10px] font-bold uppercase">{s.city}</div></div>
                                                ...
                                                <div key={idx} className="bg-white border border-slate-100 rounded-xl p-3 flex flex-col gap-2 shadow-xs">
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center gap-3">
                                                            <div className="w-6 h-6 bg-slate-100 rounded-md flex items-center justify-center text-[10px] font-black text-slate-400">{idx + 1}</div>
                                                            <span className="text-[#1d2d6a] font-bold text-xs">{s.name}</span>
                                                        </div>
                                ...
                                <div className="space-y-4">
                                    <h4 className="font-black text-[#1d2d6a] text-xs flex items-center gap-2 border-l-4 border-[#ee6f1f] pl-3 uppercase tracking-wider">Operasional & PIC</h4>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-1">
                                            ...
                                            <div className="relative">
                                                ...
                                                <input value={form.nama_pic} onChange={e => setForm({ ...form, nama_pic: e.target.value })} placeholder="Nama Lengkap" className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-3 text-[#1d2d6a] font-bold text-sm focus:outline-none focus:border-[#ee6f1f] transition-all" />
                                            </div>
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Kontak PIC</label>
                                            <div className="relative">
                                                <Phone className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={14} />
                                                <input value={form.kontak_pic} onChange={e => setForm({ ...form, kontak_pic: e.target.value })} placeholder="No. Telepon" className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-3 text-[#1d2d6a] font-bold text-sm focus:outline-none focus:border-[#ee6f1f] transition-all" />
                                            </div>
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">IP Address</label>
                                            <div className="relative">
                                                <Wifi className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={14} />
                                                <input value={form.ip_address} onChange={e => setForm({ ...form, ip_address: e.target.value })} placeholder="e.g. 192.168.1.10" className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-3 text-[#1d2d6a] font-bold text-sm focus:outline-none focus:border-[#ee6f1f] transition-all" />
                                            </div>
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Keterangan</label>
                                            <input value={form.keterangan} onChange={e => setForm({ ...form, keterangan: e.target.value })} placeholder="Catatan Operasional" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-[#1d2d6a] font-bold text-sm focus:outline-none focus:border-[#ee6f1f] transition-all" />
                                        </div>
                                    </div>
                                    <div className="space-y-1">
                                        ...
                                        <div className="relative">
                                            ...
                                            <input value={form.media} onChange={e => setForm({ ...form, media: e.target.value })} placeholder="URL Gambar/Media Kereta" className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-3 text-[#1d2d6a] font-bold text-sm focus:outline-none focus:border-[#ee6f1f] transition-all" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center justify-between border-l-4 border-[#ee6f1f] pl-3">
                                    <h4 className="font-black text-[#1d2d6a] text-xs uppercase tracking-wider flex items-center gap-2"><Layout size={14} /> Daftar Gerbong</h4>
                                    <button onClick={() => setForm({ ...form, gerbongs: [...form.gerbongs, { nama_gerbong: '', no_urut: form.gerbongs.length + 1, media: '', log_maintenance: '', log_operasional: '' }] })} className="text-[10px] font-black text-[#ee6f1f] hover:text-[#d45d15] flex items-center gap-1 uppercase tracking-widest"><Plus size={12} /> Tambah Gerbong</button>
                                </div>
                                            ...
                                            {form.gerbongs.map((g, idx) => (
                                                <div key={idx} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm space-y-4 relative group">
                                                    ...
                                                    <div className="flex gap-4">
                                                        <div className="w-10 h-10 bg-[#1d2d6a] text-white rounded-xl flex items-center justify-center font-black text-sm flex-shrink-0">{idx + 1}</div>
                                                        <div className="flex-1 grid grid-cols-2 gap-3">
                                                            <div className="space-y-1">
                                                                <label className="text-[8px] font-black text-slate-400 uppercase">Nama Gerbong</label>
                                                                ...
                                                                }} placeholder="e.g. EKSEKUTIF 1" className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-[#1d2d6a] font-bold text-xs" />
                                                            </div>
                                                            <div className="space-y-1">
                                                                <label className="text-[8px] font-black text-slate-400 uppercase">No. Urut</label>
                                                                <input type="number" value={g.no_urut} onChange={e => {
                                                                    const ng = [...form.gerbongs];
                                                                    ng[idx].no_urut = Number(e.target.value);
                                                                    setForm({ ...form, gerbongs: ng });
                                                                }} className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-[#1d2d6a] font-bold text-xs" />
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div className="grid grid-cols-2 gap-3 pt-2 border-t border-slate-50">
                                                        <div className="space-y-1">
                                                            <label className="text-[8px] font-black text-slate-400 uppercase flex items-center gap-1"><Settings size={10} /> Maintenance Log</label>
                                                            <input value={g.log_maintenance} onChange={e => {
                                                                const ng = [...form.gerbongs];
                                                                ng[idx].log_maintenance = e.target.value;
                                                                setForm({ ...form, gerbongs: ng });
                                                            }} placeholder="Status Terakhir" className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 text-[#1d2d6a] font-bold text-[10px]" />
                                                        </div>
                                                        <div className="space-y-1">
                                                            <label className="text-[8px] font-black text-slate-400 uppercase flex items-center gap-1"><Activity size={10} /> Ops Log</label>
                                                            <input value={g.log_operasional} onChange={e => {
                                                                const ng = [...form.gerbongs];
                                                                ng[idx].log_operasional = e.target.value;
                                                                setForm({ ...form, gerbongs: ng });
                                                            }} placeholder="Riwayat Perjalanan" className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 text-[#1d2d6a] font-bold text-[10px]" />
                                                        </div>
                                                    </div>
                                                    <div className="space-y-1">
                                                        <label className="text-[8px] font-black text-slate-400 uppercase flex items-center gap-1"><Info size={10} /> MediaURL</label>
                                                        <input value={g.media} onChange={e => {
                                                            const ng = [...form.gerbongs];
                                                            ng[idx].media = e.target.value;
                                                            setForm({ ...form, gerbongs: ng });
                                                        }} placeholder="URL Gambar Gerbong" className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 text-[#1d2d6a] font-bold text-[10px]" />
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                        ...
                        <div className="flex justify-end gap-3 pt-6 border-t border-slate-100">
                            <button onClick={() => setShowForm(false)} className="px-8 py-4 bg-slate-100 text-slate-500 font-black rounded-2xl text-sm transition-all active:scale-95">Batal</button>
                            <button onClick={handleAdd} disabled={saving || !form.name.trim()} className="px-12 py-4 bg-[#ee6f1f] hover:bg-[#d45d15] disabled:bg-slate-200 text-white font-black rounded-2xl text-sm transition-all flex items-center gap-2 active:scale-95 shadow-lg group">
