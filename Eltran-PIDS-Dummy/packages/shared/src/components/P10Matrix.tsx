/** /shared/src/components/P10Matrix.tsx — untuk mengubah: komponen PIDS; fungsi utama: P10Matrix */
  ...
  const updateBitmap = (textToRender: string, fixedTextToRender: string) => {
    ...
    if (fixedCtx && fixedTextToRender) {
      ...
      fixedCtx.fillText(
        fixedTextToRender,
        xOffset,
        Math.floor(LED_HEIGHT / 2) + 1,
      );

      fixedTextDataRef.current = fixedCtx.getImageData(
        0,
        0,
        fixedCanvas.width,
        LED_HEIGHT,
    ...
    fullTextDataRef.current = textCtx.getImageData(
      0,
      0,
      textCanvas.width,
      LED_HEIGHT,
