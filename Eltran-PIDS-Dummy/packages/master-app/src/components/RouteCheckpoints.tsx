...
export function RouteCheckpoints({
    ...
    const currentStationDisp = navData.find((x: any) => x.status === "BERHENTI")?.name || getStationName(data?.currentStation);
    ...
    return (
        <SectionAccordion
            ...
            icon={MapPin}
            defaultOpen={true}
            summary={!route?.name ? "Pilih Rute di Selector" : `Detail ${navData.length} POI Navigasi • Lokasi Saat Ini: ${currentStationDisp || '-'}`}
        >
                            ...
                            <div className="mt-8 border border-slate-200 rounded-[2.5rem] overflow-hidden shadow-md bg-white flex flex-col">
                                ...
                                <div className="flex-shrink-0 bg-[#1d2d6a] text-white border-b border-[#152355]">
                                    <table className="w-full text-left whitespace-nowrap border-separate border-spacing-0 table-fixed">
                                        ...
                                        <thead>
                                            <tr>
                                                <th className="py-6 px-6 text-sm font-bold uppercase tracking-widest first:rounded-tl-[2rem]">Nama Stasiun</th>
                                                <th className="py-6 px-4 text-sm font-bold uppercase tracking-widest">Ket</th>
                                                <th className="py-6 px-4 text-sm font-bold text-right uppercase tracking-widest">Longitude</th>
                                                <th className="py-6 px-4 text-sm font-bold text-right uppercase tracking-widest">Latitude</th>
                                                <th className="py-6 px-4 text-sm font-bold text-center uppercase tracking-widest">TTA</th>
                                                <th className="py-6 px-4 text-sm font-bold text-center uppercase tracking-widest">Status</th>
                                                <th className="py-6 px-6 text-sm font-bold uppercase tracking-widest last:rounded-tr-[2rem]">Next Stasiun</th>
                                            </tr>
                                        </thead>
                                    </table>
                                </div>
                                ...
                                <div ref={navTableRef} className="flex-grow overflow-x-hidden max-h-[600px] overflow-y-auto relative custom-scrollbar bg-white rounded-b-[2.5rem] pb-10">
                                    <table className="w-full text-left whitespace-nowrap border-separate border-spacing-0 table-fixed">
                                        <colgroup>
                                            <col className="w-[22%]" />
                                            <col className="w-[8%]" />
                                            ...
                                            {navData.map((item: any, idx: number) => {
                                                ...
                                                return (
                                                    <tr key={idx} data-active={isBerhenti} className={`hover:bg-slate-50 transition-colors ${isBerhenti ? 'bg-orange-50/100' : 'bg-white'}`} style={{ scrollMarginTop: '64px' }}>
                                                        <td className={`py-6 px-6 font-bold flex items-center gap-4 ${isBerhenti ? 'text-[#ee6f1f]' : 'text-slate-700'}`}>
                                                            {isBerhenti && <ChevronRight size={20} className="text-[#ee6f1f]" />}
                                                            <div className="truncate" title={item.name}>{item.name}</div>
                                                        </td>
                                                        <td className="py-6 px-4 font-bold text-slate-500 text-xs uppercase tracking-wider truncate" title={item.type}>{item.type}</td>
                                                        <td className="py-6 px-4 font-mono font-bold text-slate-600 text-sm text-right">{item.lng}</td>
                                                        <td className="py-6 px-4 font-mono font-bold text-slate-600 text-sm text-right">{item.lat}</td>
                                                        <td className="py-6 px-4 font-mono font-bold text-[#1d2d6a] text-center text-base">{item.eta}</td>
                                                        <td className="py-6 px-4 text-center">
                                                            {item.status ? (
                                                                <span className={`text-[10px] font-bold px-3 py-1.5 rounded-lg shadow-sm uppercase tracking-widest ${isBerhenti ? 'bg-[#1d2d6a] text-white' : 'bg-slate-200 text-slate-500'}`}>{item.status}</span>
                                                            ) : (<span className="text-slate-300 text-sm">-</span>)}
                                                        </td>
                                                        <td className="py-6 px-6 font-bold text-slate-500 text-xs italic truncate" title={item.next}>{item.next}</td>
                                                    </tr>
                                                );
                                            })}
                                        </tbody>
